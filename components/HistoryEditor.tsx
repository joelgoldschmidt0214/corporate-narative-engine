...
const HistoryEditor: React.FC<Props> = ({
  data,
  onUpdate,
  onProceed,
  onBack,
  ...
  const handleCellChange = (
    ...
    newData[index] = {
      ...newData[index],
      [field]:
        field === "marketContext" || field === "companyEvent"
          ? value
          : numValue,
    };
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6">
      <div className="sticky top-16 z-20 bg-white/90 backdrop-blur-md shadow-sm border-b border-slate-200 -mx-4 px-4 py-3 flex justify-between items-center mb-6">
        <button
          onClick={onBack}
          className="flex items-center gap-2 px-4 py-2 bg-white text-slate-600 border border-slate-300 rounded-lg hover:bg-slate-50 transition font-bold text-sm"
        >
        ...
        <button
          onClick={onProceed}
          className="flex items-center gap-2 px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition shadow-md hover:shadow-lg font-bold text-sm"
        >
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex gap-4 text-xs">
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 bg-blue-500 rounded-sm"></span>売上高
              (左軸)
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-1 bg-green-500"></span>営業利益 (右軸)
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-1 bg-purple-500"></span>CF (右軸)
            </span>
          </div>
        </div>
        ...
        >
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-500 font-bold border-b border-slate-200 sticky top-0 z-10 shadow-sm">
              <tr>
                <th className="px-4 py-3 sticky left-0 bg-slate-50 z-20 border-r border-slate-200 whitespace-nowrap">
                  年度
                </th>
                <th className="px-4 py-3 min-w-[100px] whitespace-nowrap">
                  売上高
                </th>
                <th className="px-4 py-3 min-w-[100px] whitespace-nowrap">
                  営業利益
                ...
                <th className="px-4 py-3 min-w-[80px] whitespace-nowrap">
                  社員数
                </th>
                ...
                <tr key={row.year} className="hover:bg-slate-50 transition">
                  ...
                  <td className="px-4 py-3">
                    <input
                      ...
                      value={row.revenue}
                      onChange={(e) =>
                        handleCellChange(index, "revenue", e.target.value)
                      }
                      className={getCellClass(index, "revenue")}
                    />
                  </td>
                  <td className="px-4 py-3">
                    <input
                      ...
                      className={`${getCellClass(index, "operatingProfit")} ${
                        row.operatingProfit < 0 ? "text-red-600 font-bold" : ""
                      }`}
                    />
                  </td>
                  <td className="px-4 py-3">
                    <input
                      ...
                      value={row.cashFlow}
                      onChange={(e) =>
                        handleCellChange(index, "cashFlow", e.target.value)
                      }
                      className={getCellClass(index, "cashFlow")}
                    />
                  </td>
                  <td className="px-4 py-3">
                    <input
                      ...
                      value={row.employees}
                      onChange={(e) =>
                        handleCellChange(index, "employees", e.target.value)
                      }
                      className={getCellClass(index, "employees")}
                    />
                  </td>
                  <td className="px-4 py-3">
                    <div className="max-h-24 overflow-y-auto">
                      <textarea
                        value={row.marketContext}
                        onChange={(e) =>
                          handleCellChange(
                            index,
                            "marketContext",
                        ...
                        className={`${getCellClass(
                          index,
                          "marketContext"
                        )} resize-none whitespace-normal min-h-[3rem] w-full`}
                      />
                    </div>
                  </td>
                  <td className="px-4 py-3">
